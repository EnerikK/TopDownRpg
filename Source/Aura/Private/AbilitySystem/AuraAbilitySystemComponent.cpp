...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			...
			if(!SlotIsEmpty(Slot)) // There is an ability in this slot already Deactivate and clear the slot
			{
				FGameplayAbilitySpec* SpecWithSlot = GetSpecWithSlot(Slot);
				if(SpecWithSlot)
				{	//Is the ability the same as this ability ? if yes return early
			...
			if(!AbilityHasAnySlot(*AbilitySpec)) // Ability Doesnt have a slot yet, meaning its not active 
			{
