                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for(auto& Pair : FAuraGameplayTags::Get().DamageTypesResistances)
	{
		...
		if(UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			//3.bind a lamda OnDamageDelegate on the victim here
			//4 call ugameplaystatics::APplyradialdamagewithfalloff to cause damage (this will result in take damagebeing called
			//on the victim which will the broadcast ondamagedelegate ? ?
			//5. In Lam,da set dmaagetype value to the damage receive from the broadcast;
